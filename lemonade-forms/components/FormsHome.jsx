    ...
    return (
      <>
        <FormEditor
          ...
          onSave={({ bytes, filename, action }) => setPending({ bytes, filename, formType: editing.formType, patient: editing.patient, action })}
          onCancel={() => setEditing(null)}
        />
        {pending && (
          <SendDialog
            pdfBytes={pending.bytes} filename={pending.filename} formType={pending.formType}
            provider={currentUser} patient={pending.patient} initialAction={pending.action}
